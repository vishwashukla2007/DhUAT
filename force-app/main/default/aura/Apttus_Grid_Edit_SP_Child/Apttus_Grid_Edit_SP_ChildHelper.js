({
	fetchpicklistvalue : function(component, helper) {
        ...
        var action = component.get("c.findPicklistOptions");
        action.setParams({
            "objAPIName":  "Specialty_Pricing__c",
        ...
        action.setParams({
            "objAPIName":  "Specialty_Pricing__c",
            "fieldAPIname": "Pharmacy_Benefit__c"
        })
        action.setCallback(this, $A.getCallback(function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                component.set("v.pharmacybenefit", response.getReturnValue());
            }
            else {
                helper.counselLogErrors(response.getError());
            }
        ...
        action.setCallback(this, $A.getCallback(function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                component.set("v.lobOp", response.getReturnValue());
            }
            else {
                helper.counselLogErrors(response.getError());
            }
    ...
    deleteSelectedRecords : function(component, event, helper) {
        var action = component.get("c.deleteSelectedSpecialtyRecords");
        action.setParams({
            "spPrId" : component.get("v.objects.Id")
        });
