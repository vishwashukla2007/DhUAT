({
        ...
        action.setCallback(this, function( response ) {    
            ...
            if (state === "SUCCESS")
            {
                ...
                if(res.success){
                    ...
                    var pbmlist = res.listPBMServices; 
                    ...
                    pbmlist.forEach(function(item) {
                        ...
                        item['PBMURL'] = '/lightning/r/PBM_Services__c/' + item['Id'] + '/view';  
                        item['PBMURLNote'] = $A.util.isUndefinedOrNull(item['Notes__c']) ?'/apex/NewPBMservice?Id=' + item['Id'] :'/lightning/r/PBM_Services__c/' + item['Id'] + '/view';  
                    ...
                    component.set("v.pbmList", pbmlist);
    ...
    handleSort: function(cmp, event) {
        ...
        var cloneData = cmp.get( "v.pbmList");//this.DATA.slice(0);
        cloneData.sort((this.sortBy(fieldsTosort, sortDirection === 'asc' ? 1 : -1)));
        
        cmp.set( "v.pbmList", cloneData);                
        ...
        return new Promise($A.getCallback(function(resolve){
            ...
            action.setCallback(this,function(response){
                ...
                if(res.success){
                    ...
                    pbmlist.forEach(function(item) {
                        item['PBMURL'] = '/lightning/r/PBM_Services__c/' + item['Id'] + '/view';
                        //item['PBMURLNote'] = '/lightning/r/PBM_Services__c/' + item['Id'] + '/view';  
                        item['PBMURLNote'] = $A.util.isUndefinedOrNull(item['Notes__c']) ?'/apex/NewPBMservice?Id=' + item['Id'] :'/lightning/r/PBM_Services__c/' + item['Id'] + '/view';
                        item['SERVICEcss'] = item['Additional_Service__c'] === false ? 'slds-cell-edit readOnly' : 'slds-cell-edit normal';
                        item['Service_Description2__c'] = item['Service_Description2__c'] ? item['Service_Description2__c'].replace( /(<([^>]+)>)/ig,'') :'';
                        item['lastModifedOn'] = item['Last_Date_Modified__c'] ? Date.parse(item['Last_Date_Modified__c']):null;
    ...
    updatedata : function(component,datalist,approve){
        ...
        component.set("v.spinner",true);
        var action = component.get("c.updatePBMService");  
        action.setParams({             
            'updatedPBMServiceList' : datalist ,
            'approve':approve
        });  
        action.setCallback( this, function( response ) {  
            var state = response.getState();   
    ...
    updatedataReview : function(component,datalist){
        component.set("v.spinner",true);
        var action = component.get("c.updatePBMServiceReview");  
        action.setParams({             
            'updatedPBMServiceList' : datalist
        });  
        action.setCallback( this, function( response ) {  
