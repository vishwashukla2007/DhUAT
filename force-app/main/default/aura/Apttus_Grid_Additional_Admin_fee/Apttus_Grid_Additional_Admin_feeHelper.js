({
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                for(var lob in loblist)  {
                    ...
                    helper.loadData1(component, event, helper);
                    helper.loadData2(component, event, helper);
                }
            } else {
    ...
    retrievelob: function(component, event, helper) {
        ...
        action1.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS")  {
                ...
                for(var lob in loblist)  {
                    console.log('loblist[lob].LobId__c'+loblist[lob].LobId__c);
                ...
                toastEvent.setParams({
                    ...
                    "type":"error",
                    "mode": "pester",
                    "message": "Problem in displaying Admin Fee Lob."
                });
                toastEvent.fire(); 
            }
        });
     ...
     loadData : function(component, event, helper) 
    {
        ...
        component.set("v.LobName",selectedLOB.Additional_LOB_Numeric_ID__c);
        component.set("v.lobdesc",selectedLOB.Additional_LOB_ID__c);
        component.set("v.fafid",selectedLOB.Additional_FAF_ID__c);
        var action = component.get("c.get_AdminWrapperList");
        action.setParams({"fafid":  selectedLOB.Additional_FAF_ID__c})
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
	  ...
	  loadData1 : function(component, event, helper) 
    {
		var selectedLOB = component.get("v.selectedLOB");
        if(selectedLOB && selectedLOB.Additional_FAF_ID__c){  
        component.set("v.LobName",selectedLOB.Additional_LOB_Numeric_ID__c);
        component.set("v.lobdesc",selectedLOB.Additional_LOB_ID__c);
        component.set("v.fafid",selectedLOB.Additional_FAF_ID__c);
        var action = component.get("c.get_Admin340WrapperList");
        action.setParams({"fafid":  selectedLOB.Additional_FAF_ID__c})
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                component.set("v.wrapperList340", response.getReturnValue());
                 var count = response.getReturnValue();
 				   var itemcount = count.length;
                   if (itemcount == 0)
                   {
	   ...
	   loadData2 : function(component, event, helper) 
    {
		var selectedLOB = component.get("v.selectedLOB");
        if(selectedLOB && selectedLOB.Additional_FAF_ID__c){  
        component.set("v.LobName",selectedLOB.Additional_LOB_Numeric_ID__c);
        component.set("v.lobdesc",selectedLOB.Additional_LOB_ID__c);
        component.set("v.fafid",selectedLOB.Additional_FAF_ID__c);
        var action = component.get("c.get_AdminManualWrapperList");
        action.setParams({"fafid":  selectedLOB.Additional_FAF_ID__c})
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                component.set("v.wrapperListManual", response.getReturnValue());
                 var count = response.getReturnValue();
 				   var itemcount = count.length;
                   if (itemcount == 0)
                   {
    ...
    createmlob: function(component, event, helper){
        ...
        processAddLobLogic: function(component, event, helper) {
        var selectlob = component.find("selectlob");
        var changeValue = selectlob.get("v.value");
        if(changeValue && changeValue != "none"){
            var action = component.get("c.createMultipleLob");
            action.setParams({
                "Basealiid" : component.get("{!v.ali}"),
                "basefafId" : component.get("v.basefafId"),
                "baseLobId" :  component.get("v.baseLobId"),
                "baselobDesId" :  component.get("v.baselobDesId"),
                "selectedLobId" : changeValue            
            });
            action.setCallback(this, function(response) {
                var state = response.getState();
                if (state === "SUCCESS") {
