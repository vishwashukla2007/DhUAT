({
  ...
  console.log('Handle Save '+component.get("{!v.desc}"));
            ...
            if (state === "SUCCESS") 
               {
                ...
                appEvent.setParams({ "Grid" : "ELECTRONIC"});
                appEvent.fire();
                }
                else
                {
   ...
   if(component.get("v.display") == false)
    ...
    deleteSelectedRec : function(component, event, helper) {
        ...
        component.set("{!v.display}", false);
         ...
         appEvent.setParams("message","ELECTRONIC");
         appEvent.fire();
