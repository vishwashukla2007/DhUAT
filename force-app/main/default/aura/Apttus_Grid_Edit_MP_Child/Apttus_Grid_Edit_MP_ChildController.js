({
    ...
    yr.push({
     ...
     if (component.get("{!v.objects.Brand_Basis__c}") != "Baked-Preferred & Non-Preferred")
        ...
        //alert('Custom Description'+component.get("v.desc"));
    ...
    handleChange : function(component, event, helper) {
        ...
        var inputcmp112 = component.find("input112");
        var value = inputcmp112.get("v.value");
   ...
   if(component.get("v.display") == true)
        ...
        var inputcmp5 = component.find("input5");
        var value = inputcmp5.get("v.value");
        if((value == null || value =="") && component.get("v.flatrequired")==true && value !="0")
        {
        ...
        var inputcmp11 = component.find("input11");
        var value = inputcmp11.get("v.value");
        if((value == null || value =="") && component.get("v.nonmacrequired")==true && value !="0")
        {
        ...
        if (Req1 == 1 || Req2 == 1 || Req3 == 1 || Req4 == 1 || Req5 == 1 || Req6 == 1 || Req7 == 1 || Req8 == 1 || Req9 == 1 || Req10 == 1 ||Req11==1)
        {
        ...
        if(Req1 == 0 && Req2 == 0 && Req3 == 0 && Req4 == 0 && Req5 == 0 && Req6 == 0 && Req7 == 0 && Req8 == 0 && Req9 == 0 && Req10 == 0 && Req11==0)
        {
           //alert('Description-->'+component.get("{!v.objects.Custom_Description__c}"));
          var action = component.get("c.get_savemaildata");
          action.setParams({ "mp" : component.get("{!v.objects}")});
            ...
            if (state === "SUCCESS") 
               {
                ...
                appEvent.setParams({ "Mail" : component.get("v.objects")});
                appEvent.setParams({ "Grid" : "Mail"});
                appEvent.fire();
                }
                else
                {
                ...
                var mailobj = component.get("v.objects");
                appEvent.setParams({ "SaveRec" : "false"});
                appEvent.setParams({ "Mail" : mailobj});
                appEvent.setParams({ "Grid" : "Mail"});
                appEvent.fire();
               /* message = "Mail Pricing record already exists for the Year, LOB combination. Please create a unique record using the combination.";  
                var appEvent = $A.get("e.c:Apttus_Grid_NT_Notify_Error_Event");
   ...
   handleduplicate : function(component, event, helper) 
    {
        ...
        if (component.get("v.objects.Year__c") == yr && component.get("v.objects.LOB__c") == lob)
        {
                var cmpTarget = component.find("tablerow");
                $A.util.addClass(cmpTarget, "slds-has-error");
                var message = "Mail Pricing record already exists for the Year, LOB combination. Please create a unique record using the combination.";  
                var appEvent = $A.get("e.c:Apttus_Grid_NT_Notify_Error_Event");
                appEvent.setParams({ "Error" : message});
                appEvent.fire();
        }
